-- Title-case venue names with standard English title-case rules and trim
-- trailing whitespace. Mirrors the song title-casing migration
-- (20260413225404_fix_song_title_casing); slug is left untouched so existing
-- /venues/<slug> links keep working.
...
BEGIN
  ...
  -- Don't touch all-caps names (e.g., "MSG")
...
-- Apply title casing and trim whitespace on all venues where the result differs
UPDATE venues
SET name = pg_temp.title_case(TRIM(name))
WHERE name != pg_temp.title_case(TRIM(name));
